...
| **CLAUDE.md** | 工作說明檔案——按位置與配置載入 | Ch 13, 14 |
...
| **session** | 會話——一段有記錄的工作對話，可退出後恢復 | Ch 3 |
| **shell** | 命令直譯器——解析並執行命令的程式 | Ch 1 |
| **Skill** | 技能包——按需載入的任務手冊 | Ch 20 |
| **slash command** | 斜槓命令——`/` 開頭的快捷指令 | Ch 18, 21, 附錄 C |
| **subagent** | 子智慧體——派出去的專業實習生 | Ch 22 |
| **terminal** | 終端——承載文字輸入與輸出的介面，多個系統都有 | Ch 1 |
| **token** | 模型處理資訊的單位——與漢字、單詞沒有固定換算比例 | Ch 12, 15 |
| **tool use** | 工具呼叫——AI 使用外部工具的能力 | Ch 22, 26 |
| **vibe review** | 對低風險改動的快速初篩，不能替代必要測試 | Ch 10 |
