...
| **CLAUDE.md** | 工作说明文件——按位置与配置加载 | Ch 13, 14 |
...
| **session** | 会话——一段有记录的工作对话，可退出后恢复 | Ch 3 |
| **shell** | 命令解释器——解析并执行命令的程序 | Ch 1 |
| **Skill** | 技能包——按需加载的任务手册 | Ch 20 |
| **slash command** | 斜杠命令——`/` 开头的快捷指令 | Ch 18, 21, 附录 C |
| **subagent** | 子智能体——派出去的专业实习生 | Ch 22 |
| **terminal** | 终端——承载文字输入与输出的界面，多个系统都有 | Ch 1 |
| **token** | 模型处理信息的单位——与汉字、单词没有固定换算比例 | Ch 12, 15 |
| **tool use** | 工具调用——AI 使用外部工具的能力 | Ch 22, 26 |
| **vibe review** | 对低风险改动的快速初筛，不能替代必要测试 | Ch 10 |
