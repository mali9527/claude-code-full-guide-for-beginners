...
**做什么**：退出普通前台会话（输入 `/exit`，先检查后台任务），重新 `claude` 启动。
...
`@目录/` 提供文件列表，不会自动加入全部正文；但要求“读完里面所有文件”仍可能导致大量读取。先指定任务相关的文件，例如 `@src/components/Button.tsx`，再按需要补材料。
...
- **Ctx% 用的就是 token 比例**
- **API 用量按相应 token 费率计算**；订阅额度与额外用量分开看（第 15 章）
