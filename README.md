...
**2026 年 9 月公开预览稿**：内容复核截至 **2026-09-25**，已纳入 **Opus 5.5、Fable 5.1**，新模型示例使用 Claude Code **2.1.280 或更新版本**。本稿已逐章核对易变事实、命令与练习，并采用 **42 张手绘插图**。真实账号及 Windows 实机验证仍待完成，具体范围见[版本与核验说明](docs/版本与核验说明.md)。
...
- **想连起来读**：[简体合订稿](全书.md) · [繁體閱讀入口](zh-TW/README.md)。繁体由本次源稿转换，语言审读待完成。
...
当前开放的是经过事实与编辑复核的九月预览稿；完整平台实测尚未完成，暂不登记为正式里程碑。旧的章节地址保留；第 0 章与附录 J 的历史文件名不代表其当前内容。

[四月历史 PDF](pdf-build/output/Claude-Code-零基础入门指南.pdf)仍保留原入口。**该 PDF 不包含本次九月修订**，不要据它查询新模型、当前费用和权限行为；九月 PDF 按固定源提交另行导出；本入口仍是四月文件。
...
内容清单和引用关系由 `book.yaml` 管理，事实来源在 `facts.yaml`；本仓工具快照可独立运行。先保存源稿，再生成合订稿与繁体。私有任务和完整研究记录留在系列总控，不进入书仓。
