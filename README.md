...
简体中文 · [English overview](README.en.md)（英文导览；正文目前为中文）

**不会编程，也能从第一次对话开始，学会让 AI 读文件、改文件，处理日常工作。** 从打开终端讲起，Mac 与 Windows 分开说明，遇到报错有地方查。作者：**马力**。

**[开始阅读](manuscript/00-前言/00-本书怎么读.md)** · [按需找章节](#全书目录) · [常见报错](manuscript/附录/G-报错自救手册.md) · [系列教程](docs/系列教程.md)

![Claude Code 零基础入门指南：从第一次对话，到处理日常工作](assets/social/cover-bilingual-20260925.png)

你可以先学会三件事：把任务说清楚；让它读取和修改文件；在授权、核对和回退时知道该做什么。需要扩展时，再进入 Skills、子代理、Hooks 与 MCP。

觉得有用，欢迎点右上角 **Star** 收藏，方便下次查阅。想接收正式版本更新，可选择 **Watch → Custom → Releases**。
...
| 已经装好，想开始做事 | [第一次对话](manuscript/part1-从零起步/03-你的第一次对话.md) → [让它读文件](manuscript/part2-日常使用/05-让它读文件.md) |
| 担心它改错文件 | [权限机制](manuscript/part2-日常使用/07-让它跑命令加权限机制.md) → [计划与回退](manuscript/part2-日常使用/09-计划模式与撤销.md) |
| 读过旧版，回来查变化 | [九月更新导读](docs/2026-09更新导读.md) |

也可打开[简体合订稿](全书.md)连续阅读，或按[手绘插图索引](assets/illustrations/读图索引.md)查找概念。偏好图形界面，可看[桌面应用导览](manuscript/附录/H-桌面应用导览.md)。

本书免费阅读；**使用 Claude Code 的账号与模型费用另计**。免费 Claude 聊天计划不自动包含 Code。本教程由作者独立编写。

<details>
<summary>当前版本与核验范围 · 2026 年 9 月简体正式版</summary>

内容复核截至 **2026-09-25**，已纳入 **Opus 5.5、Fable 5.1**，新模型示例使用 Claude Code **2.1.280 或更新版本**。本稿已逐章核对易变事实、命令与练习，并采用 **42 张手绘插图**。本次以官方文档复核作为操作说明的发行依据，未宣称完成 Mac / Windows 真实账号实测，具体范围见[版本与核验说明](docs/版本与核验说明.md)。

</details>

## 全书目录

前言 + 第 0—27 章 + 附录 A—K。先读「主线」，需要时再回到「支线」。
...
朋友正在从零学习 Claude Code，可以把本页发给他；如果是在解决某个具体问题，直接分享对应章节更方便。后续公开教程统一收录在[系列教程](docs/系列教程.md)，每本保留独立仓库与阅读入口。
