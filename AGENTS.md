...
作品 claude-code。本书不是 Codex 教程。当前为 2026-09 简体发行稿。作者于 2026-09-25 明确授权：以官方文档核对操作说明，修正明显问题后发布简体版；本轮不要求真实电脑实测，暂不发行繁体。其他产品仍待独立任务。
...
资料核验用内置 Web；官方定产品事实，社区教程只补使用经验并注明来源。先保存受审源提交，再写 checks；编辑复核可由当前引擎完成，与真实操作和事实确认分别记录。另一实际引擎交叉审核默认不启用，仅由作者手动触发；未触发或不可用不影响推进、合入和发布。实测未知仍如实保留。本次简体发行按作者新标准要求 editorial 与 facts，细节见 docs/2026-09-25简体发行复核.md；不得把文档核对填成 operations 通过。当前任务已获合入 main、推送与正式发行授权；后续任务仍按各自授权处理，不自动发帖或注册购买。
