...
一本面向完全零基础读者（不会编程、没用过终端）的 Claude Code 系统入门书。规模约 8 万字（中文字 + 英文词）、15-20 小时阅读时长、28 章（前言 + Ch 0-27）+ 11 附录（**第 0 章 = 国内零成本免费跑通**；附录 J = Opus 4.7 专题；附录 K = 第三方模型接入）。权威需求在 `需求文档.md`（v2.1）。
